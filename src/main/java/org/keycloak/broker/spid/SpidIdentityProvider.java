...
import org.keycloak.broker.spid.logging.LogCollector;
...
public class SpidIdentityProvider extends AbstractIdentityProvider<SpidIdentityProviderConfig> {
	...
	public Response performLogin(AuthenticationRequest request) {
		try {
			...
			if (getConfig().getConfig().get(SpidIdentityProviderConfig.ALLOW_CREATE) == null
					|| getConfig().isAllowCreate())
				allowCreate = Boolean.TRUE;
			SAML2AuthnRequestBuilder authnRequestBuilder = new SAML2AuthnRequestBuilder()
					.assertionConsumerUrl(assertionConsumerServiceUrl).destination(destinationUrl)
					.issuer(SAML2NameIDBuilder.value(issuerURL)
							...
							.setFormat(JBossSAMLURIConstants.NAMEID_FORMAT_ENTITY.get()).build())
					...
					.nameIdPolicy(SAML2NameIDPolicyBuilder.format(nameIDPolicyFormat)
							...
							.setSPNameQualifier(issuerURL).setAllowCreate(allowCreate))
			...
			request.getAuthenticationSession().setClientNote(SPID_REQUEST_ISSUE_INSTANT,
	...
	public void authenticationFinished(AuthenticationSessionModel authSession, BrokeredIdentityContext context) {
		...
		if (subType != null) {
			...
			authSession.setUserSessionNote(SpidSAMLEndpoint.SAML_FEDERATED_SUBJECT_NAMEID,
					subjectNameID.serializeAsString());
		}
		AuthnStatementType authn = (AuthnStatementType) context.getContextData()
				.get(SpidSAMLEndpoint.SAML_AUTHN_STATEMENT);
	...
	public void backchannelLogout(KeycloakSession session, UserSessionModel userSession, UriInfo uriInfo,
		...
		if (singleLogoutServiceUrl == null || singleLogoutServiceUrl.trim().equals("")
				|| !getConfig().isBackchannelSupported())
			return;
		...
		try {
			...
			int status = SimpleHttp.doPost(singleLogoutServiceUrl, session)
					.param(GeneralConstants.SAML_REQUEST_KEY,
							binding.postBinding(SAML2Request.convert(logoutRequest)).encoded())
	...
	public Response keycloakInitiatedBrowserLogout(KeycloakSession session, UserSessionModel userSession,
		...
		if (getConfig().isBackchannelSupported()) {
			...
			try {
				LogoutRequestType logoutRequest = buildLogoutRequest(userSession, uriInfo, realm,
	...
	protected LogoutRequestType buildLogoutRequest(UserSessionModel userSession, UriInfo uriInfo, RealmModel realm,
			String singleLogoutServiceUrl, NodeGenerator... extensions) throws ConfigurationException {
		String entityId = getEntityId(uriInfo, realm);

		SAML2LogoutRequestBuilder logoutBuilder = new SAML2LogoutRequestBuilder()
				.assertionExpiration(realm.getAccessCodeLifespan()).issuer(SAML2NameIDBuilder.value(entityId)
						// SPID: Aggiungi l'attributo NameQualifier all'elemento Issuer
				...
				.nameId(NameIDType
						.deserializeFromString(userSession.getNote(SpidSAMLEndpoint.SAML_FEDERATED_SUBJECT_NAMEID)))
	...
	private JaxrsSAML2BindingBuilder buildLogoutBinding(KeycloakSession session, UserSessionModel userSession,
			RealmModel realm) {
		JaxrsSAML2BindingBuilder binding = new JaxrsSAML2BindingBuilder(session).relayState(userSession.getId());
		if (getConfig().isWantAuthnRequestsSigned()) {
			KeyManager.ActiveRsaKey keys = session.keys().getActiveRsaKey(realm);
			String keyName = getConfig().getXmlSigKeyInfoKeyNameTransformer().getKeyName(keys.getKid(),
					keys.getCertificate());
	...
	public Response export(UriInfo uriInfo, RealmModel realm, String format) {
		try {
			...
			// Create the AttributeConsumingService if at least one attribute importer
			// mapper exists
			...
			realm.getIdentityProviderMappersByAliasStream(getConfig().getAlias()).forEach(mapper -> {
				...
				if (target instanceof SamlMetadataDescriptorUpdater)
					metadataAttrProviders.add(
							new java.util.AbstractMap.SimpleEntry<>(mapper, (SamlMetadataDescriptorUpdater) target));
			});
			...
			if (!metadataAttrProviders.isEmpty()) {
				...
				if (attributeConsumingServiceName == null)
					attributeConsumingServiceName = realm.getDisplayName() != null ? realm.getDisplayName()
							: realm.getName();
